import { resolveTypstOptions, type TypstInputMode, type TypstMode, type TypstRendererOptions } from './config';
...
export async function preRenderTypstMarkdown(
  ...
  output = await replaceAsync(output, TYPST_MATH_FENCE_RE, async (match, source) => {
    return renderMarkdownTypst(source, options.blockMode ?? 'block', 'math', resolvedOptions, match);
  });

  output = await replaceAsync(output, TYPST_INLINE_RE, async (match, source) => {
    return renderMarkdownTypst(source, options.inlineMode ?? 'inline', 'math', resolvedOptions, match);
  });
